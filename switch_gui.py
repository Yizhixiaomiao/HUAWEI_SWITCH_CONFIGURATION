...
class HuaweiSwitchGUI:
    def __init__(self, root):
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)  # 捕获关闭事件
    ...
    def start_sync(self):
        ...
        devices = [{'ip': ip, 'username': username, 'password': password, 'port': port} for ip in ips]

        self.btn_start.config(state=tk.DISABLED)
        threading.Thread(target=self.run_backup, args=(devices, max_workers), daemon=True).start()
    ...
    def run_backup(self, devices, max_workers):
        ...
        future_map = {self.executor.submit(
            self.backup_tool.backup_single_switch, d['ip'], d['username'], d['password'], d['port']
        ...
        try:
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    logger.info(message)
                    self.log(f"{'✅ 成功' if success else '❌ 失败'}: {message}")
                except Exception as e:
    ...
    def generate_config_table(self):
        ...
        threading.Thread(target=self.process_configs, args=(backup_dir, output_file), daemon=True).start()
    ...
    def parse_filename(self, filename):
        ...
        if len(parts) < 4:
            return None
        system_name, switch_model, ip_address, timestamp = parts
        ...
        return {"系统名称": system_name, "交换机型号": switch_model, "IP地址": ip_address, "提取时间": formatted_time}

    def check_8021x_config(self, file_path):
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            return 'domain sangfor_802.1x' if re.search(r'domain\s+sangfor_802\.1x', content) else '未刷入'
        except Exception as e:
    ...
    def get_cfg_files(self, directory):
        return [f for f in os.listdir(directory) if f.endswith('.cfg') and os.path.isfile(os.path.join(directory, f))]
    ...
    def on_close(self):
        if not self.is_running:
            self.root.destroy()
            return
        if messagebox.askyesno("确认关闭", "有任务正在同步，是否强制停止并退出？"):
            try:
                if self.executor:
                    self.executor.shutdown(cancel_futures=True)
            except:
                pass
            os._exit(0)  # 强制退出，避免后台线程残留
    ...
    def connect_to_switch(self, ip, username, password, port=22, timeout=15):
        ...
        try:
            ...
            client.connect(hostname=ip, port=port, username=username, password=password,
                           timeout=timeout, banner_timeout=25, auth_timeout=20, look_for_keys=False, allow_agent=False)
            return client
        except Exception as e:
